...
module.exports = HandleMsg = async (urbae, message) => {
	try {
		...
		// [IDENTIFY]
		const ownerNumber = "6281281922793@c.us"
		...
		const mess = {
			...
			wait: '_Tunggu ya ;D    sedang proses_',
    ...
    const menupict = "https://yt3.ggpht.com/jrobELASuKy-r4AISt4xEJHLV56DCHcERPQBlZY2y5ptUi5wg7ZWF5CMvur-yvYXrzUATxCf3A=s176-c-k-c0x00ffffff-no-rj"
		...
		if (isCmd && isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwnerB || isPrem) {
			switch (command) {
				...
				case prefix + 'tnc':
					...
					break
				case prefix + 'help':
					const bots = `Hi ${pushname}, this is Paimong, to find out the commands menu, type *${prefix}menu* , *${prefix}p*`
					...
					if (isPrem) {
						await urbae.joinGroupViaLink(linkgrup)
							.then(async () => {
								...
								await urbae.sendText(chekgrup.id, `what up y'all , I'm Paimong. To find out the commands on this Bot type ${prefix}menu`)
							})
					} else {
				...
				case prefix + 'ytmp3':
					...
					yt.ytMp3(q)
						.then(result => {
							console.log(result)
							urbae.sendFileFromUrl(from, result.thumb, 'thumb.jpg', `「 *YouTube* 」\n\n*Title:* ${result.title}\n*Views:* ${result.views}\n*Channel:* ${result.channel}\n*Uploaded:* ${result.published}\n\n${mess.sendfileaudio}`, id)
							hxzapi.youtube(q)
								.then(res => {
				...
				case prefix + 'distance':
					...
					break
				case prefix + 'glitch':
					if (args.length === 0) return urbae.reply(from, `Kirim perintah *${prefix}glitch [ Teks1|Teks2 ]*, contoh *${prefix}glitch |Paimong|Sal*`, id)
					try {
						urbae.reply(from, mess.wait, id)
				...
				case prefix + 'add':
					...
					if (!isBotGroupAdmins) return urbae.reply(from, 'Gagal, kalo mau pake fitur ini, jadiin aku admin', id)
				...
				case prefix + 'promote':
					...
					if (!isBotGroupAdmins && !isOwnerB) return urbae.reply(from, 'Gagal, kalo mau pake fitur ini jadiin aku admin', id)
...
_Desc di update oleh : @${chat.groupMetadata.descOwner.replace('@c.us', '')} pada *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* pukul ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
				...
				case prefix + 'addbacot': {
					if (!args.length >= 1) return urbae.reply(from, 'BACOTAN NYA MANA??!', id)
